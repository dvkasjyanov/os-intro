...
1.   Прочитаю справку `man` команды `kill` (Рисунок 15), после чего использую её для завершения процесса `gedit` (Рисунок 16).
...
1.   Получаю более подробную информацию о командах `df` и `du` с помощью команды `man` (Рисунок 17). 
...
1.  Воспользовавшись справкой команды `find`, вывожу имена всех директорий, имеющихся в моем домашнем каталоге. Для этого использую команду `find ~ -maxdepth 1 -type d` (Рисунок 20).
